...
def activate_app(thread_name=""):
    ...
    if not (app.isActive()):
        ...
        if not (activationResult):
            print(f"{thread_name}: app activation failed")
...
def capture_image_thread():
    ...
    """
    while(app):
        print(f"capture_image_thread: Has looped {capture_image_thread_statistics.count} times. Elapsed time is {capture_image_thread_statistics.get_time()}")
        activate_app("capture_image_thread")
        print("capture_image_thread: Grabbing a screenshot")
...
def infer_image_thread():
    ...
    while(True):
        ...
        if(image != None):
            print("infer_image_thread: Has looped {} times. Elapsed time is {}".format(infer_image_thread_statistics.count, infer_image_thread_statistics.get_time()))
            print("infer_image_thread: inferring image from latest screenshot using ollama")
            payload = {
                "model": "llava",
            ...
            print("infer_image_thread: There is not a latest screenshot to infer from")
            time.sleep(1)
...
def controller_input_thread():
    ...
    """
    while(controller):
        # TODO: uncomment logs (they're noisy).
        # TODO: Only enter if the right window is active. (Annoying that keystrokes are entered while debugging)
        # print(f"controller_input_thread: Has looped {controller_input_thread_statistics.count} times. Elapsed time is {controller_input_thread_statistics.get_time()}")

        # press, release, tap to send input to the controller. Joystick movement is special.
        # print("controller_input_thread: tapping Cross button")
